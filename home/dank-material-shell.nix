...
in
{
  ...
  config = lib.mkIf (osConfig.vexos.desktop.environment == "hyprland") {
    ...
    # ── Idle display blanking (DPMS only — no lock) ─────────────────────────
    # modules/hyprland-desktop.nix omits hypridle to avoid double-locking with
    # DMS's own lock screen. This adds it back for exactly one purpose: turning
    # the display off after inactivity. No lock_cmd is set anywhere below, so
    # this daemon never locks the session — system-nosleep.nix already disables
    # suspend/hibernate/idle-lock system-wide, and that stays untouched.
    services.hypridle = {
      enable = true;
      settings = {
        listener = [
