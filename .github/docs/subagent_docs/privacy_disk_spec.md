# Privacy Disk Automation — Implementation Specification
...
The module manages tmpfs-rooted impermanence for the privacy role. It currently contains
...
### Privacy host files (`hosts/privacy-{amd,nvidia,intel,vm}.nix`)

Minimal — import `configuration-privacy.nix` and a GPU module only. No disk configuration.
...
All privacy configs use `commonModules` which imports `/etc/nixos/hardware-configuration.nix`.
...
The privacy role requires non-standard disk layout (LUKS2 + Btrfs subvolumes + tmpfs root)
...
privacy role's target audience (Tails/privacy users who may not be NixOS experts).
...
- `modules/privacy-disk.nix` — a new NixOS module using **disko** to declare the
  full GPT + EFI + LUKS2 + Btrfs subvolume layout declaratively
- `scripts/privacy-setup.sh` — a new interactive bash script for initial disk setup
...
```
User runs scripts/privacy-setup.sh from NixOS live ISO
    │
    ...
    │
    └─ nixos-install --flake /mnt/etc/nixos#vexos-privacy-<variant>
         └─ NixOS evaluates flake:
              ├─ /etc/nixos/hardware-configuration.nix (hw details, no fileSystems)
              ├─ modules/privacy-disk.nix (disko → generates fileSystems + LUKS config)
...
| Path | Purpose |
|---|---|
| `modules/privacy-disk.nix` | Disko-based disk layout module with Nix options |
| `scripts/privacy-setup.sh` | Interactive initial-install script for privacy role |
| `template/privacy-disko.nix` | Standalone parameterized disko config (used by setup script) |
...
| `hosts/privacy-amd.nix` | Import `modules/privacy-disk.nix`; set `vexos.privacy.disk.device` |
| `hosts/privacy-nvidia.nix` | Import `modules/privacy-disk.nix`; set `vexos.privacy.disk.device` |
| `hosts/privacy-intel.nix` | Import `modules/privacy-disk.nix`; set `vexos.privacy.disk.device` |
| `hosts/privacy-vm.nix` | Import `modules/privacy-disk.nix`; set device + `enableLuks = false` |
| `scripts/install.sh` | Add notice for privacy role directing users to `privacy-setup.sh` |

---

## 9. New Nix Options

Module: `modules/privacy-disk.nix`
Option namespace: `vexos.privacy.disk`

| Option | Type | Default | Description |
|---|---|---|---|
| `vexos.privacy.disk.device` | `str` | `"/dev/nvme0n1"` | Block device for the privacy disk layout. Passed to disko as `disko.devices.disk.main.device`. |
| `vexos.privacy.disk.enableLuks` | `bool` | `true` | Wrap the data partition in LUKS2. Set `false` for VMs. |
| `vexos.privacy.disk.luksName` | `str` | `"cryptroot"` | Name of the LUKS device-mapper entry (`/dev/mapper/<luksName>`). |

---

## 10. Disko Module Design: `modules/privacy-disk.nix`

```nix
# modules/privacy-disk.nix
# Declarative disk layout for the VexOS privacy role.
#
...
Replace with:
> "Disk layout is handled by modules/privacy-disk.nix (disko). The tmpfs root
> is declared by this module automatically when enabled."

---

## 12. Privacy Host File Changes
...
{ lib, ... }:
{
  ...
  vexos.privacy.disk.device = "/dev/nvme0n1";
...
Same pattern as AMD — add `modules/privacy-disk.nix` import and `vexos.privacy.disk.device`.
...
{ inputs, ... }:
{
  ...
  vexos.privacy.disk.device     = "/dev/vda";
  vexos.privacy.disk.enableLuks = false;
...
**Note:** The existing bootloader comment block in privacy-vm.nix can be removed —
...
`modules/privacy-disk.nix` can access `inputs.disko.nixosModules.disko` directly. The explicit
destructuring is for documentation clarity and consistency with other inputs.)*

---

## 14. Setup Script Design: `scripts/privacy-setup.sh`

This script is run from the **NixOS live ISO** to perform an initial privacy role installation.
...
6. Prompt: "Enter hostname [vexos-privacy]:"
...
10. Run nixos-install:
    sudo nixos-install \
      --no-root-passwd \
      --flake /mnt/etc/nixos#vexos-privacy-${VARIANT}
...
## 15. Standalone Disko Template: `template/privacy-disko.nix`

A standalone parameterized disko config used by `scripts/privacy-setup.sh`.
This is **separate** from `modules/privacy-disk.nix` (which is a NixOS module).
...
# Used by scripts/privacy-setup.sh during initial installation.
# Parameters:
...
When `ROLE = "privacy"`, print a notice before the `nixos-rebuild switch` step:

```bash
# After selecting role = "privacy", before GPU variant selection:
echo ""
...
| Concern | Hardware (AMD/NVIDIA/Intel) | VM (privacy-vm) |
|---|---|---|
...
| `privacy-setup.sh` support | Yes | Yes (skips LUKS steps) |

For the VM, the setup script detects `VARIANT = "vm"` and passes `--arg enableLuks false` to disko.

The VM's `privacy-vm.nix` removes the existing bootloader comment block — bootloader is
...
After this implementation, privacy role hardware-configuration.nix should contain:
...
For users migrating existing privacy installs (where hw-config was manually edited),
...
| disko doesn't set `neededForBoot = true` → impermanence bind mounts fail silently | HIGH | Explicitly set `fileSystems."/persistent".neededForBoot = lib.mkForce true` in privacy-disk.nix |
| disko input adds nixpkgs duplication | MEDIUM | `inputs.nixpkgs.follows = "nixpkgs"` declared in flake input |
| Desktop/HTPC/server hosts accidentally import privacy-disk.nix | LOW | privacy-disk.nix only imported in privacy host files — not in commonModules |
...
| `/persistent` has `neededForBoot` | Fails if user missed this flag | Always passes — privacy-disk.nix sets it via `lib.mkForce true` |
