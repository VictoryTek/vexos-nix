...
- [x] **M-30** `[A]` `HEAVY_BUILD_REGEX` defined in three places with two different values — will drift again
  ...
  - **Resolution:** File references were stale (the script moved from `modules/nix.nix` to `pkgs/vexos-update/default.nix` during M-26 this session), and all occurrences already held identical values — but the structural duplication remained. Found two real duplicates: (1) `pkgs/vexos-update/default.nix` defined the same kernel-modules regex twice under two names (`KERNEL_BLOCK_REGEX`, `HEAVY_BUILD_REGEX`) — consolidated into one definition, hoisted above its first use. (2) `UNAVOIDABLE_REGEX` duplicated between `scripts/install.sh` and `pkgs/vexos-update/default.nix`. A true sourced-fragment fix was infeasible: `install.sh`'s documented, primary usage is `curl -fsSL ... | bash` with no local repo clone present, so it cannot source a file from `scripts/`. Confirmed this constraint with the user, who chose to keep the literal duplicate (required for the curl-pipe install path) but add cross-reference comments at both definition sites so a future edit to one is prompted to check the other. (`pkgs/vexos-update/default.nix`, `scripts/install.sh`)
...
- [x] **M-32** `[F]` Add qBittorrent + Bazarr opt-in options to the arr stack
  ...
  - **Resolution:** Added both as nested sub-options of the existing `vexos.server.arr` module (the plan's own requested option paths confirm this — `arr.nix` already bundles 5 services in one file, a pre-existing deviation from the one-file-per-service convention, out of scope to fix here). qBittorrent's `webuiPort` defaults to 8080 upstream, colliding with SABnzbd's default — shifted to 8081. Its `torrentingPort` defaults to `null` (random port each run, unfirewallable) — fixed to 6881 (conventional BitTorrent port). Both sub-toggles assert their parent `vexos.server.arr.enable = true`, placed outside the `lib.mkIf` block so the assertion actually fires (verified in Phase 3) rather than silently no-op-ing. (`modules/server/arr.nix`)

- [x] **M-33** `[F]` Caddy LAN reverse-proxy layer — service names and local TLS instead of 40 raw ports
  - **Source:** FEATURES 2.5
  ...
  - **Resolution:** Added `modules/server/proxy.nix` (`vexos.server.proxy.enable`, asserting `vexos.server.caddy.enable`) generating a `<service>.<hostname>.local` Caddy virtualHost for every enabled service across a ~45-entry hand-maintained table, cross-checked against each service's actual port (vexos option or upstream module default). Caddy's built-in non-public-hostname handling gives these local TLS automatically. Confirmed with the user to skip Avahi/mDNS publication entirely — the existing mDNS setup (`network.nix`/`network-desktop.nix`) is flagged as hard-won/fragile SMB-discovery infrastructure in standing project notes, so these names require the operator's own DNS/hosts resolution rather than a new mDNS record set. Verified via `extendModules` that the generated virtualHost set exactly matches enabled services with correct ports. (`modules/server/proxy.nix`, `modules/server/default.nix`)

- [x] **M-34** `[F]` cockpit-zfs NAS Phase B — reserved one-line landing site in `nas.nix`, blocked on upstream packaging
  ...
  - **Resolution:** Re-checked directly against this repo's pinned nixpkgs rather than trusting the old comment. `pkgs.cockpit-zfs` now exists (1.2.27-3, not `meta.broken`) — the original Yarn Berry v4 workspace-dependency blocker is gone. Actually built it to confirm rather than trusting attribute existence alone: it still fails, now due to an unrelated upstream Tailwind/PostCSS error in the shared `@45drives/houston-common-ui` workspace. Since it's still not buildable, per the plan's own "if buildable" condition, no `pkgs/cockpit-zfs/`, sub-option, or `nas.nix` line was added — updated both files' deferral comments with the current, specific blocking reason instead of the stale one. Confirmed via `extendModules` that `vexos-server-amd`/`vexos-headless-server-amd` produce byte-identical `.drv` hashes before and after, proving zero behavioral change to the working Samba/NFS configuration these files also contain. (`modules/server/cockpit.nix`, `modules/server/nas.nix`)

- [x] **M-35** `[F]` `just update-containers` — `:latest` containers never actually update without an explicit pull
  - **Source:** FEATURES 3.1
  ...
  - **Resolution:** User correctly flagged this as mostly already resolved by M-22 (7 of 8 OCI containers pinned + the Wednesday `update-container-images.yml` auto-bump workflow) and asked that updates ride the existing `just update`/`update-all`/`deploy` commands rather than a separate recipe — confirmed both points directly rather than assuming. Found exactly one remaining `:latest` straggler (`arcane.nix`, added after M-22's pass) — pinned to `v1.19.4` (verified against GHCR's actual published tags) and added it to `update-container-images.yml`'s tracked list. No new `just` recipe: since every image tag is now a literal string in this repo's own source, `nix flake update` + `nixos-rebuild switch` (what `update`/`update-all`/`deploy` already do) picks up any pin change automatically — the same mechanism the other 7 already rely on. (`modules/server/arcane.nix`, `.github/workflows/update-container-images.yml`)

- [x] **M-36** `[F]` CI should build and push `pkgs/*` custom packages to the Attic cache the modules advertise
  - **Source:** FEATURES 4.1 · `modules/nix.nix:25-46`
  - Add CI workflow job: `nix build .#<pkg outputs>`, then `attic push` via repo-secret token
  - **Resolution:** Investigated first — there is no project-owned Attic cache today (`vexos.attic.cacheUrl` defaults to `null`; it's a client-side option for each operator's own self-hosted instance), so there's no stable URL or repo secret for CI to push to. User then asked whether the existing wiring is even correct if enabled later — verified both `modules/nix.nix` (client substituter/key) and `modules/server/attic.nix` (server, `dataDir` matching atticd's own `StateDirectory`) directly against upstream `atticd` module source; both already correct, no changes needed. The real gap was that nothing pushes packages into a cache at all — added `just attic-push [cache]`, which builds all 7 `pkgs.vexos.*` packages and pushes each independently (one package's failure doesn't abort the rest — caught and fixed a real bug in the recipe itself, plus found and worked around a pre-existing, intentional `lib.fakeHash` placeholder in `kiji-proxy` that requires `just enable kiji-proxy` first). No CI workflow or secret added, per user's confirmed preference to scope this to local use until they decide to run a persistent instance. (`justfile`, `modules/server/attic.nix`)

- [x] **M-37** `[F]` NixOS VM boot test to gate `nixpkgs-unstable` bumps (the manual procedure the README prescribes)
  - **Source:** FEATURES 4.2
  ...
  - **Resolution:** `pkgs.nixosTest` is now a hard `throw` at this pinned nixpkgs rev (renamed to `testers.nixosTest`) — confirmed directly against source rather than assumed. Added `checks.x86_64-linux.gnome-boot` reusing `vexos-desktop-vm`'s exact real module composition (minus the impure `hardware-configuration.nix`), invoked only as a single named target (`nix build .#checks.x86_64-linux.gnome-boot`) — never via forbidden `nix flake check`. Hit and fixed two real evaluation conflicts building it: `modules/network.nix`'s `mkDefault` hostname collided with the testing framework's own same-priority default, and NixOS unconditionally forbids `nixpkgs.config.*` once `pkgs` is externally supplied (as `nixosTest` does) — both fixed via `lib.mkForce` overrides scoped to the test node only, zero change to the real modules. Ran the test to a full, actual pass (not just eval) — it genuinely boots a VM, confirms GDM/graphical.target, and checks the real signal a "black screen" regression breaks (the Wayland session socket actually appearing). Wired into `README.md`'s existing manual bump procedure as an added automated step; kept the manual VM check as an optional final step. Verified via identical `.drv` hashes that the new check has zero effect on any real `nixosConfigurations` output. (`flake.nix`, `README.md`)
...
- [x] **L-02** `[B]` justfile default recipe claims "vexos" default password and tells user to edit the wrong file
...
- [x] **L-06** `[B]` `stateless-setup.sh` writes the password hash world-readable (0644) and git-adds it
  - **Source:** BUGS L6 · `scripts/stateless-setup.sh:29-32`
  - Change `sudo tee` → `install -m 0600` for `stateless-user-override.nix`
  - **Resolution:** Kept the `tee` heredoc (needed for inline content generation) and added `chmod 0600` immediately after each write, plus `cp -p` on the later persisted-copy sites so the tightened mode survives. Same defect (identical unguarded `tee`) also existed in `scripts/migrate-to-stateless.sh`'s existing-install migration path — fixed there too, since it handles the same class of secret. Verified nothing else in the repo (`vexos-update`, `install.sh`) rewrites the file with a permissive mode afterward — both only `git add -f` it, which doesn't touch filesystem permissions. Preflight (Phase 6) could not run in this session's environment (no Nix on the Windows host); syntax-checked via `bash -n` instead, and the user ran preflight/pushed independently. (`scripts/stateless-setup.sh`, `scripts/migrate-to-stateless.sh`)

- [x] **L-07** `[B]` Install scripts fetch moving refs mid-run (disko `latest`, install.sh `main` chain)
  ...
  - **Resolution:** Verified disko's `/latest` ref directly against upstream — it's disko's own maintained "current stable" tag and their documented recommended invocation, not a repo-introduced bug; per user decision, left unchanged rather than pinning it to a fixed tag (which would diverge from upstream's own guidance and add manual-bump burden). The real defect was this repo's own script chain: `install.sh` hands off to `stateless-setup.sh`/`migrate-to-stateless.sh` via `curl main/... | bash` mid-run, and `stateless-setup.sh` separately downloads two templates from `main` — each fetch independently resolving `main` at the moment it runs, across a process that can span several minutes, with no guarantee all fetches see the same commit. Fixed by resolving a single commit SHA once (`git ls-remote`, with the same nix-fallback-install pattern already used for `openssl`/`git` elsewhere in these scripts) as early as possible, exported as `VEXOS_REV`, and reused for every download in that run — including explicitly passing it through the `sudo` boundary (`sudo VEXOS_REV="..." bash`) for the migrate-to-stateless.sh hand-off, since sudo's `env_reset` would otherwise silently drop a plain exported variable. `stateless-setup.sh` only resolves its own `VEXOS_REV` if not already inherited, so the whole `install.sh` → sub-script chain pins to one commit. `migrate-to-stateless.sh` needed no functional change (it performs no further downloads). (`scripts/install.sh`, `scripts/stateless-setup.sh`)

- [x] **L-08** `[B]` `zfs-server.nix` installs `pkgs.zfs` alongside the module-managed build — potential version skew
  - **Source:** BUGS L8 · `modules/zfs-server.nix:50-55`
  - Use `config.boot.zfs.package` instead of `pkgs.zfs` in `environment.systemPackages`
  - **Resolution:** Verified directly against the upstream ZFS NixOS module (at this repo's pinned nixpkgs rev) that `boot.zfs.package` (default `pkgs.zfs`) is the supported override point for the ZFS userland build, and that nothing in this repo currently overrides it — so today the two references are only accidentally identical. Replaced the hardcoded `pkgs.zfs` list entry with `config.boot.zfs.package` so this stays correct if the option is ever pinned to a different build (matching the same instinct this file already applies to `boot.kernelPackages`). This Windows session has no local Nix, but a WSL2 Ubuntu distro with Nix 2.34.1 was found and used to genuinely validate: `nix flake show --impure`, the three mandatory desktop dry-build-equivalents, `vexos-server-amd`/`vexos-headless-server-amd` (via `extendModules`-injected throwaway hostId, since both ship only placeholder values by design per M-13), and a full `bash scripts/preflight.sh` run — all passed. (`modules/zfs-server.nix`)

- [x] **L-09** `[B]` `gnome-flatpak-install` installs all apps in one transaction — one bad ID fails everything
  - **Source:** BUGS L9 · `modules/gnome-flatpak-install.nix:67-70`
  - Mirror the per-app loop from `modules/flatpak.nix`
  - **Resolution:** Replaced the single multi-app `flatpak install` call with a per-app `for` loop plus a `FAILED` flag, mirroring `modules/flatpak.nix`'s already-shipped pattern exactly. This also fixed a worse, previously-undocumented sibling defect: the completion stamp was written unconditionally regardless of install success, so a partial failure was never retried; the stamp write is now gated on `FAILED=0`, with a distinct `.gnome-last-failed-install` marker (kept separate from `flatpak.nix`'s own marker since both services run on the same GNOME-role hosts) on the failure path. The pre-existing `extraRemoves` migration-uninstall block was already per-app-isolated and left untouched. Validated via WSL2 Nix across every role that actually imports this module (`gnome-desktop`, `gnome-htpc`, `gnome-server`, `gnome-stateless` — desktop-amd/nvidia/vm, htpc-amd, stateless-amd, server-amd all evaluated cleanly) plus a full `preflight.sh` PASS. (`modules/gnome-flatpak-install.nix`)

- [x] **L-10** `[B]` `kernel-install-override` timing window: override file deleted in exit-2 path, not restored
  ...
  - **Resolution:** This logic no longer lives in `modules/nix.nix` (M-26 moved it to `pkgs/vexos-update/default.nix`). Traced all exit paths reachable after the override file's early deletion: the real gap wasn't the `exit 2` classifier path (which never touches this file) but `exit 1` — if the kernel-cache-status dry-build check itself errors out for any reason unrelated to caching, the just-deleted override was never restored, silently and permanently losing the installer's kernel safety-net. Factored the override's static content into a `write_kernel_override()` shell function (avoiding duplicating it) and called it both from the existing recreate-if-still-heavy branch and the new restore-on-check-failure branch. Validated via WSL2 Nix: rebuilt `pkgs.vexos.vexos-update` directly (exercises ShellCheck) and ran the full `preflight.sh` — both passed. (`pkgs/vexos-update/default.nix`)

- [x] **L-11** `[B]` `programs.git` ships `user.email = ""` — git refuses to commit with an explicit empty value
  - **Source:** BUGS L12 · `home/bash-common.nix:13-16`
  - Remove the `user.email` key entirely
  - **Resolution:** Re-verified the literal claim directly — tested `git commit` with a real `user.name` and empty `user.email` (this repo's exact combination); the commit succeeds (records `Name <>`), it does not refuse. Git's hard "empty ident" failure triggers on an empty *name*, not email — the plan's title didn't reproduce, but the underlying problem (malformed blank-email authorship on every commit under this identity, and suppressing git's own "please configure your identity" fallback/warning, which only fires when the key is truly absent) was still real and worth fixing. Removed the `email = lib.mkDefault "";` line entirely, leaving `name` untouched. Confirmed zero interaction risk: this repo's own automated git commits (`install.sh`, `vexos-update`) already pass their own explicit `-c user.email=/-c user.name=`, independent of this file. Validated broadly via WSL2 Nix across every role importing this file (desktop-amd/nvidia/vm, stateless-amd, htpc-amd, server-amd, headless-server-amd) plus a full `preflight.sh` PASS. (`home/bash-common.nix`)

- [x] **L-12** `[B]` `nvidia-vaapi-driver` incorrectly gated on `latest` only — excludes `legacy_535` users who could use it
...
- [x] **L-15** `[A]` `distroName` priority arithmetic re-derived at every site — needs a single option with documented precedence
