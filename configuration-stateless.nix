...

{
  ...
  # bare-metal-install.sh and migrate-to-stateless.sh both write a real password
  ...
  # Runs after the `users` script so the user already exists when chown runs.
  system.activationScripts.statelessDocuments = {
    deps = [ "users" ];
    text = ''
      DIR="/persistent/home/${config.vexos.user.name}/Documents"
      if [ ! -d "$DIR" ]; then
        ${pkgs.coreutils}/bin/mkdir -p "$DIR"
        ${pkgs.coreutils}/bin/chown ${config.vexos.user.name}:users "$DIR"
      fi
    '';
  };
  ...
  # (hashedPassword = "!").  An operator must run scripts/bare-metal-install.sh
  ...
  warnings = lib.optional
    ...
    ''
      ...
      Run scripts/bare-metal-install.sh to set a password, or manually create
