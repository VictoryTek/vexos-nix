...
#     and stored at dataDir/secrets/home-registry-env (0600, root-only),
#     along with a fully-assembled DATABASE_URL (the app reads DATABASE_URL
#     as a single connection string, not discrete POSTGRES_HOST/USER/PORT
#     vars like Joplin does). Set vexos.server.home-registry.environmentFile
#     yourself only if you want to manage the secret through another backend
#     (e.g. sops-nix).
#   - The Postgres user (postgres) and database name (home_inventory) are
#     hardcoded to match the upstream image, which does not currently read
#     env vars to override either — see
...
in
{
  options.vexos.server.home-registry = {
    ...
    environmentFile = lib.mkOption {
      ...
      description = ''
        ...
        and DATABASE_URL=<full connection string>, shared by both the
        home-registry and home-registry-db containers. Leave unset (the
        default) to have a random password generated automatically on first
        activation at dataDir/secrets/home-registry-env — no manual secret
  ...
  (lib.mkIf cfg.enable {
    ...
    # Auto-generates the Postgres password (and the DATABASE_URL Home
    # Registry actually reads) on first activation when the operator hasn't
    # supplied their own environmentFile — this is what makes
    # vexos.server.home-registry.enable = true; sufficient on its own, with
      ...
      script = ''
        ...
        if [ ! -f "${effectiveEnvFile}" ]; then
          dbPass=$(${pkgs.openssl}/bin/openssl rand -hex 24)
          {
            ...
            echo "DATABASE_URL=postgres://postgres:$dbPass@home-registry-db:5432/home_inventory"
          } > "${effectiveEnvFile}"
